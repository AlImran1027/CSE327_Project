...
from .utils import ConfigManager, PaymentFacade, RegularPricing, PremiumPricing, PricingContext
from .models import FlightProxy

@check_permission('app_name.view_resource')
def protected_view(request):
    return HttpResponse("This is a protected view.")

def form_view(request, user_type):
    FormClass = form_factory(user_type)
    form = FormClass()
    return render(request, 'form_template.html', {'form': form})

def config_view(request):
    config = ConfigManager()
    config.set_setting('example_key', 'example_value')
    value = config.get_setting('example_key')
    return HttpResponse(f"Config value: {value}")

def proxy_view(request, flight_id):
    proxy_flight = FlightProxy.objects.get(id=flight_id)
    discounted = proxy_flight.discounted_price(10)
    return HttpResponse(f"Discounted price: {discounted}")

def payment_view(request):
    result = PaymentFacade.process_payment(100, 'credit')
    return HttpResponse(result)

def price_view(request):
    user_is_premium = getattr(request.user, 'is_premium', False)
    strategy = PremiumPricing() if user_is_premium else RegularPricing()
    context = PricingContext(strategy)
    price = context.get_final_price(100)
    return HttpResponse(f"The final price is {price}")
